...
My design uses the linear actuator- IMA 33 MV43 which has a max force of 11.1 kiloNewtons(kN). The bar was already lifted an initial height of 21.52 centimeters(cm) due to the initial height of the base of the linear actuator. The left edge of the bar is connected by a pin suport to the left side of the space we're given and the linear actuator is pinned all the way at the right edge of the space. The last pin is one between the end of the linear actuator's piston that extends and the linear bar. Please refer to the image below to get a visual of the design space. 

<img src="{{ '/assets/images/HW 4- Portfolio Design.jpg' | relative_url }}" alt="Portfolio Design">
